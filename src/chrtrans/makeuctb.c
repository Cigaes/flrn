...
int getunicode (char **p0)
{
	...
	return -2;
	       ...
	       !isxdigit((int) p[2]) || !isxdigit((int) p[3]) || !isxdigit((int) p[4]) ||
	       !isxdigit((int) p[5]) || isxdigit((int) p[6])) {
	return -1;
    }
    *p0 = p+6;
...
void addpair_str (char *str, int un)
{
   int i;
...
char this_FLRNcharset[UC_MAXLEN_FLRNCSNAME +1];
...
int main (int argc, char **argv)
{
	...
	switch (*p) {
		...
		for (i = 0; *p && i < UC_MAXLEN_FLRNCSNAME; p++, i++) {
		    this_FLRNcharset[i] = *p;
		}
		this_FLRNcharset[i] = '\0';
		continue;
	...
	*p = TOLOWER(*p);
    ...
    if (argc >= 4) {
	strncpy(this_FLRNcharset,argv[3],UC_MAXLEN_FLRNCSNAME);
    } else if (this_FLRNcharset[0] == '\0') {
	strncpy(this_FLRNcharset,this_MIMEcharset,UC_MAXLEN_FLRNCSNAME);
    }
/***** DO NOT produce trailing spaces!
    if ((i = strlen(this_FLRNcharset)) < UC_LEN_FLRNCSNAME) {
	for (; i < UC_LEN_FLRNCSNAME; i++) {
	    this_FLRNcharset[i] = ' ';
	}
	this_FLRNcharset[i] = '\0';
    }
*******/
    ...
    fprintf(stderr,"this_FLRNcharset: %s.\n",this_FLRNcharset);
#endif /* NOTDEFINED */
	...
	*id_append = '\0';
	...
	for (i = 0, p = this_MIMEcharset;
	     *p && (i < UC_MAXLEN_ID_APPEND-1);
	     p++, i++) {
	    id_append[i+1] = isalnum((int) *p) ? *p : '_';
	}
	id_append[i+1] = '\0';
    }
    fprintf(stderr, " (%s).\n", id_append);
...
id_append, this_MIMEcharset, this_FLRNcharset,
